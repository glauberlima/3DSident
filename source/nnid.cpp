...
namespace NNID {
    ...
    const char* GetAccountId() {
        ...
        if (R_FAILED(ret =
                         ACT_GetAccountInfo(accountId, sizeof(accountId), ACT_DEFAULT_ACCOUNT, INFO_TYPE_ACCOUNT_ID))) {
            Log::Error("%s failed: 0x%x\n", __func__, ret);
            return "unknown";
        }
    ...
    const char* GetCountryName() {
        ...
        if (R_FAILED(ret = ACT_GetAccountInfo(countryName, sizeof(countryName), ACT_DEFAULT_ACCOUNT,
