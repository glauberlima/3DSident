...
namespace Kernel {
    const char* GetInitialVersion() {
        ...
        if (R_FAILED(ret = FSFILE_Read(handle, std::addressof(bytesRead), 0, reinterpret_cast<u32*>(buf),
    ...
    const char* GetVersion(VersionInfo info) {
        ...
        std::snprintf(kernelString, 128, "%lu.%lu-%lu", GET_VERSION_MAJOR(osVersion), GET_VERSION_MINOR(osVersion),
                      GET_VERSION_REVISION(osVersion));

        std::snprintf(firmString, 128, "%lu.%lu-%lu", GET_VERSION_MAJOR(osVersion), GET_VERSION_MINOR(osVersion),
                      GET_VERSION_REVISION(osVersion));

        if (R_FAILED(ret =
                         osGetSystemVersionDataString(nver, cver, systemVersionString, sizeof(systemVersionString)))) {
