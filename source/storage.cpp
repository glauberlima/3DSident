...
namespace Storage {
    ...
    u64 GetFreeStorage(FS_SystemMediaType mediaType) {
        FS_ArchiveResource const resource = Storage::GetResource(mediaType);
    ...
    u64 GetClusterSize() {
        FS_ArchiveResource const resource = Storage::GetResource(SYSTEM_MEDIATYPE_SD);
    ...
    void GetStorageForMedia(FS_SystemMediaType mediaType, u64& freeOut, u64& usedOut, u64& totalOut) {
        FS_ArchiveResource const resource = Storage::GetResource(mediaType);
        u64 const clusterSize = static_cast<u64>(resource.clusterSize);
